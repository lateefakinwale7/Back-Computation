...
st.set_page_config(layout="wide", page_title="Survey Tool", page_icon="📐")
st.title("📐 Universal Survey Computation & Audit Tool")

# --- LICENSE CHECK (Simple Version for EXE) ---
# You can change 'ACCESS-2025' to your desired key
if "unlocked" not in st.session_state:
    st.session_state.unlocked = False

if not st.session_state.unlocked:
    st.sidebar.header("🔑 Activation")
    key = st.sidebar.text_input("Enter License Key", type="password")
    if key == "ACCESS-2025":
        st.session_state.unlocked = True
        st.sidebar.success("Software Activated")
        st.rerun()
    else:
        st.info("Please enter your license key in the sidebar to begin.")
        st.stop()
...
project_notes = st.sidebar.text_area("Site Notes", placeholder="Project details...")

# --- DATA PROCESSING ---
file = st.file_uploader("Upload CSV or Excel Survey Data", type=["csv", "xlsx"])

if file:
    df_raw = pd.read_csv(file) if file.name.endswith('.csv') else pd.read_excel(file)
    
    if not df_raw.empty:
        # Step 1: Compute Observations (Safe for your coordinate file)
        df_processed = compute_lat_depart(df_raw)
        
        # Step 2: Apply Bowditch and Loop Closure
        ...
        with t_audit:
            st.subheader("Trigonometric Proof")
            ...
            st.subheader("Coordinate Accumulation")
            st.table(df_final[['code', 'Math_N', 'Final_N', 'Math_E', 'Final_E']])
        ...
        with e1:
            ...
            st.download_button("Download PDF", pdf, "Survey_Audit.pdf")
        with e2:
